...
def num_unique_questions_answered_true(input)
  input.sum do |group|
    group.split.join.split('').uniq.join.length
  end
end

def num_questions_group_answered_true(input)
  input.sum do |group|
    answers = []

    group.split.map.with_index do |line, index|
      if index == 0
        answers = line.split('')
      else
        answers &= line.split('')
...
p num_unique_questions_answered_true(input)
p num_questions_group_answered_true(input)
